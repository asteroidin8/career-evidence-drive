...
const CS_REVIEW={
  aiFeedback:'단순 CS 처리보다 문제 인지 → 원인 확인 → 유관부서 확인 → 대안 처리 → 동료 공유 → 시스템 허점 발견의 흐름이 강점. 개인 숙련도 문제인지 시스템 공통 문제인지 범위를 확장해 확인한 점이 좋음. 결과에는 확인된 사실, 실제 성과, 예정된 후속조치가 섞여 있어 분리 필요. 타 PPC 10건 이상은 반드시 유지할 객관적 근거. 현재 면접 활용도는 높음이며, 실제 프로세스 개선이나 기준 변경까지 이어지면 핵심 후보로 승격 가능.',
  aiRevision:{
    situation:'기존 고객센터를 통한 재패킹 프로세스가 자동 주문 및 회수입고·회수이동 방식으로 개편됨. 교환에 따른 재패킹 및 고객 요청 회수 건이 인입되었으나 시스템상 회수입고 처리가 되지 않았고, 개편된 CS 처리 방식에 대한 숙련 부족까지 겹쳐 업무 지연이 발생함.',
    action:'초기에는 신규 프로세스 숙련도 문제일 가능성이 있다고 판단하여 고객센터에 1차 문의하고 캡틴에게 2차 확인함. 캡틴을 통해 변경된 CS 프로세스를 다시 숙지한 뒤에도 정상 처리가 되지 않아 유관부서에 추가 확인함. 확인 결과 해당 건은 시스템상 미회수 상태였으며, 동일한 교환 회수입고 문의가 타 PPC에서도 10건 이상 발생하고 있음을 확인함. 정상적인 신규 프로세스로 처리가 어려운 상황에서 레거시 프로세스를 적용해 조정입고 후 양품화까지 완료함. 이후 풀타임 크루에게 해당 사례를 공유하고 변경된 신규 프로세스를 다시 안내함.',
    result:'- 해당 교환 건 정상 처리 및 양품화 완료\n- 개인 숙련도 문제가 아니라 신규 회수 프로세스/시스템에서 공통적으로 발생하는 문제임을 확인\n- 타 PPC에서도 유사 문의 10건 이상 발생한 사실 확인\n- 풀타임 크루에게 사례와 신규 프로세스를 공유하여 동일 상황 발생 시 참고할 수 있도록 함',
    followUp:'캡틴을 통해 교환 회수 건의 신규 프로세스 처리 기준 및 시스템 예외 상황에 대해 추가 확인 예정.'
  },
  aiRecommendedSkills:['상황 판단','문제 해결','교육/지원','프로세스 이해'],
  aiRecommendedValue:'높음',
  aiPortfolioNote:'문제 해결·변화 대응·프로세스 이해 사례. 후속 확인 결과 실제 기준 개선 또는 운영 변경이 발생하면 핵심 후보로 승격 가능.',
  reviewStatus:'followup',
  aiReviewedAt:'2026-09-06T14:30:00+09:00'
};
...
function migrateV2(){
  ...
  const i=list.findIndex(x=>x.date==='2026-09-06'&&((x.process||'').includes('교환')||(x.situation||'').includes('회수입고')));
  if(i>=0&&!list[i].aiFeedback) list[i]={...list[i],...CS_REVIEW};
...
$('#entryForm').addEventListener('submit',async e=>{
  ...
  if(driveToken){
    try{await pushRecordToDrive(item);await pushMasterBackup();statusText='로컬 + Google Drive 저장 완료';}
    catch(err){statusText=`로컬 저장 완료 · Drive 실패: ${friendlyError(err)}`;}
  }else{statusText='로컬 저장 완료 · Drive는 현재 미연결';}
  const wasNew=!$('#editId').value;
  if(wasNew) resetForm(); else $('#saveBtn').textContent='수정 저장';
...
window.applyAiSuggestion=id=>{const list=entries(),i=list.findIndex(x=>x.id===id);if(i<0)return;const x=list[i];if(x.aiRecommendedSkills?.length)x.skills=[...new Set([...(x.skills||[]),...x.aiRecommendedSkills])];if(x.aiRecommendedValue)x.value=x.aiRecommendedValue;list[i]=x;saveEntries(list);renderEntries();renderReview();openItem(id)};
window.pullOneReview=async id=>{if(!driveToken){alert('먼저 Google Drive를 연결하세요.');return}const x=entries().find(v=>v.id===id);if(!x?.driveFileId)return;try{const changed=await pullReviewForItem(x);alert(changed?'새 AI 검토본을 반영했습니다.':'새 AI 검토 댓글이 없습니다.');renderEntries();renderReview()}catch(e){alert(friendlyError(e))}};
...
function renderReview(){
  ...
  $('#typeStats').innerHTML=counts.length?counts.map(([t,c])=>`<div class="review-row"><div class="review-head"><span>${esc(t)}</span><span>${c}</span></div><div class="bar"><i style="width:${c/max*100}%"></i></div></div>`).join(''):'<div class="empty">이번 달 기록이 없습니다.</div>';
...
function updateDriveUi(){
  ...
  $('#driveBadge').textContent=connected?'DRIVE ON':restoring?'DRIVE 연결 중':'DRIVE OFF';
  $('#driveBadge').classList.toggle('connected',connected);
  $('#connectDriveBtn').disabled=!!authPending;
  $('#connectDriveBtn').textContent=authPending?'연결 중...':cfg.reconnectEnabled?'Google Drive 다시 연결':'Google Drive 연결';
  $('#driveSetupNote').className=`notice ${connected?'good':'warn'}`;
  $('#driveSetupNote').textContent=connected?'Google Drive 세션이 연결되었습니다. 저장 시 로컬 + Drive에 함께 저장됩니다.':restoring?'이전 Google Drive 연결을 복구하고 있습니다.':cfg.reconnectEnabled?'자동 연결을 사용할 수 없거나 세션이 만료되었습니다. Google Drive 다시 연결을 누르세요. 로컬 기록은 유지됩니다.':'Drive 연결 전입니다. OAuth Web Client ID를 저장하고 연결하세요. 앱은 drive.file 권한을 요청합니다.';
  $('#driveFolderInfo').textContent=cfg.folderId?`Drive 폴더 ID: ${cfg.folderId}`:'아직 앱 전용 Drive 폴더가 생성되지 않았습니다.';
}
function requestDriveAuth(silent=false){
  ...
  const fail=()=>{if(generation!==authGeneration)return;authGeneration++;driveToken=null;finish(false);$('#syncStatus').textContent='연결을 완료하지 못했습니다. Google Drive 다시 연결을 누르세요. 팝업 차단 또는 Google 로그인 상태를 확인하세요.'};
  ...
  try{
    tokenClient=google.accounts.oauth2.initTokenClient({
      ...
      callback:resp=>{
        ...
        finish(true);
        $('#syncStatus').textContent=persisted?'Google Drive 연결 완료.': '연결 완료. 브라우저 저장소가 차단되어 다음 실행 시 설정이 유지되지 않을 수 있습니다.';
      }
    });
...
async function createMultipartFile(name,parentId,text){
  const boundary='career_evidence_'+Math.random().toString(36).slice(2);const metadata={name,parents:[parentId],mimeType:'text/markdown'};
  const body=new Blob([`--${boundary}\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n${JSON.stringify(metadata)}\r\n--${boundary}\r\nContent-Type: text/markdown; charset=UTF-8\r\n\r\n`,text,`\r\n--${boundary}--`]);
  return driveFetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,modifiedTime',{method:'POST',headers:{'Content-Type':`multipart/related; boundary=${boundary}`},body});
}
async function updateMediaFile(fileId,text){return driveFetch(`https://www.googleapis.com/upload/drive/v3/files/${encodeURIComponent(fileId)}?uploadType=media&fields=id,name,modifiedTime`,{method:'PATCH',headers:{'Content-Type':'text/markdown; charset=UTF-8'},body:text})}
async function renameDriveFile(fileId,name){return driveFetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?fields=id,name`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})})}
async function pushRecordToDrive(item){
  const folderId=await ensureDriveFolder(),name=`${item.date}_${safeName(item.process)}.md`,md=toMarkdown(item);let meta;
  if(item.driveFileId){if(item.driveFileName&&item.driveFileName!==name)await renameDriveFile(item.driveFileId,name);meta=await updateMediaFile(item.driveFileId,md)}else{meta=await createMultipartFile(name,folderId,md)}
  const list=entries(),i=list.findIndex(x=>x.id===item.id);if(i>=0){list[i]={...list[i],driveFileId:meta.id||item.driveFileId,driveFileName:name,driveSyncedAt:nowIso(),driveModifiedTime:meta.modifiedTime||''};saveEntries(list);Object.assign(item,list[i])}return item;
}
async function findOrCreateBackupFile(){
  const cfg=getCfg(),folderId=await ensureDriveFolder();if(cfg.backupFileId)return cfg.backupFileId;
  const q=encodeURIComponent(`name='career-evidence-backup.json' and '${folderId}' in parents and trashed=false`);const found=await driveFetch(`https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id,name)&pageSize=10`);let id=found.files?.[0]?.id;
  if(!id){const created=await createMultipartFile('career-evidence-backup.json',folderId,JSON.stringify(entries(),null,2));id=created.id}setCfg({backupFileId:id});return id;
}
async function pushMasterBackup(){const id=await findOrCreateBackupFile();await driveFetch(`https://www.googleapis.com/upload/drive/v3/files/${encodeURIComponent(id)}?uploadType=media`,{method:'PATCH',headers:{'Content-Type':'application/json; charset=UTF-8'},body:JSON.stringify(entries(),null,2)})}
...
async function pullReviewForItem(item){
  if(!item.driveFileId)return false;
  const data=await driveFetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(item.driveFileId)}/comments?fields=comments(id,content,createdTime,modifiedTime,resolved)&pageSize=100`);
  const matches=(data.comments||[]).map(c=>({c,d:parseReviewComment(c.content,item)})).filter(x=>x.d).sort((a,b)=>new Date(b.c.modifiedTime||b.c.createdTime)-new Date(a.c.modifiedTime||a.c.createdTime));
  if(!matches.length)return false;const {c,d}=matches[0];if(item.driveReviewCommentId===c.id)return false;
  const list=entries(),i=list.findIndex(x=>x.id===item.id);if(i<0)return false;
  list[i]={...list[i],aiFeedback:d.feedback||list[i].aiFeedback||'',aiRevision:d.revision||list[i].aiRevision||null,aiRecommendedSkills:Array.isArray(d.recommendedSkills)?d.recommendedSkills:(list[i].aiRecommendedSkills||[]),aiRecommendedValue:d.recommendedValue||list[i].aiRecommendedValue||'',aiPortfolioNote:d.portfolioNote||list[i].aiPortfolioNote||'',reviewStatus:d.status||'reviewed',aiReviewedAt:d.reviewedAt||c.modifiedTime||c.createdTime||nowIso(),driveReviewCommentId:c.id};saveEntries(list);await pushRecordToDrive(list[i]);return true;
}
async function pullAllReviews(){let count=0;for(const x of entries()){if(!x.driveFileId)continue;try{if(await pullReviewForItem(x))count++}catch(e){console.warn('review pull failed',x.id,e)}}return count}
async function syncAll(){
  if(driveBusy)return;driveBusy=true;$('#syncStatus').textContent='Drive 동기화 중...';
  try{await ensureDriveFolder();const list=entries();let pushed=0;for(const x of list){await pushRecordToDrive(x);pushed++;await sleep(80)}await pushMasterBackup();const reviews=await pullAllReviews();$('#syncStatus').textContent=`완료: ${pushed}개 기록 백업, 새 AI 검토 ${reviews}개 반영`;renderEntries();renderReview()}catch(e){$('#syncStatus').textContent=`동기화 실패: ${friendlyError(e)}`}finally{driveBusy=false}
}
$('#syncDriveBtn').onclick=()=>driveToken?syncAll():alert('먼저 Google Drive를 연결하세요.');
$('#pullReviewsBtn').onclick=async()=>{if(!driveToken){alert('먼저 Google Drive를 연결하세요.');return}$('#syncStatus').textContent='AI 검토 댓글 확인 중...';try{const n=await pullAllReviews();$('#syncStatus').textContent=n?`${n}개의 새 AI 검토본을 반영했습니다.`:'새 AI 검토본이 없습니다.';renderEntries();renderReview()}catch(e){$('#syncStatus').textContent=`확인 실패: ${friendlyError(e)}`}};

const REVIEW_REQUEST=`내 Google Drive에서 Career Evidence 폴더의 최신 미검토 경력 기록(review_status: raw)을 찾아 검토해줘.\n\n원본 기록은 절대 수정하지 말고, 실제 사실과 과장을 구분해서 평가해줘. 다음을 해줘:\n1. 경력 소재 가치 평가\n2. 강점과 맹점\n3. 빠진 객관적 근거/숫자\n4. 추천 역량과 면접 활용도\n5. 상황→행동→결과→후속확인 구조의 수정본\n6. 캡틴 지원/향후 물류·운영 이직 활용 메모\n\n검토가 끝나면 해당 Drive 파일에 댓글을 하나 추가해. 댓글은 반드시 아래 형식의 평문이어야 해.\nCAREER_EVIDENCE_AI_REVIEW\n{\n  "entryId": "파일 frontmatter의 id",\n  "reviewedAt": "ISO 날짜시간",\n  "status": "reviewed 또는 followup 또는 final",\n  "recommendedSkills": ["상황 판단", "문제 해결"],\n  "recommendedValue": "낮음/보통/높음/핵심 후보 중 하나",\n  "feedback": "피드백 전문",\n  "revision": {\n    "situation": "수정 상황",\n    "action": "수정 행동",\n    "result": "수정 결과",\n    "followUp": "수정 후속 확인"\n  },\n  "portfolioNote": "활용 메모"\n}\nJSON 밖의 문장은 댓글에 넣지 마. 같은 entryId의 기존 AI 검토 댓글이 있어도 최신 검토는 새 댓글로 남겨줘.`;
