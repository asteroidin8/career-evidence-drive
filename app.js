...
function renderEntries(){
  ...
  $('#entries').innerHTML=list.length?list.slice(0,visibleEntries).map(x=>{
    const id=esc(x.id),title=esc(x.process||'제목 없음');
    const icon=(name)=>'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">'+({edit:'<path d="m16 3 5 5-12 12-6 1 1-6Z"/><path d="m14 5 5 5"/>',refresh:'<path d="M20 7v5h-5M4 17v-5h5"/><path d="M6 7a7 7 0 0 1 12-1l2 6M4 12l2 6a7 7 0 0 0 12-1"/>',trash:'<path d="M3 6h18M9 6V3h6v3M5 6l1 15h12l1-15M10 10v7M14 10v7"/>',cloud:'<path d="M7 18H6a4 4 0 0 1-1-8 7 7 0 0 1 13-2 5 5 0 0 1 0 10h-1"/><path d="m9 15 2 2 4-4"/>',clock:'<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/>',check:'<circle cx="12" cy="12" r="9"/><path d="m8 12 3 3 5-6"/>',pending:'<circle cx="12" cy="12" r="9"/><path d="M8 12h8"/>',alert:'<path d="m12 3 10 18H2Z"/><path d="M12 9v5M12 17h.01"/>'}[name])+'</svg>';
    const state=x.syncConflict?'동시 수정 확인 필요':reviewLabel(x.reviewStatus);
    const statusIcon=x.syncConflict?'alert':x.reviewStatus==='followup'?'clock':['reviewed','final'].includes(x.reviewStatus)?'check':'pending';
    return `<article class="item record-card"><button class="card-open" data-card-open="${id}" aria-label="${title} 상세 보기"></button><div class="card-toolbar"><div class="card-states">${x.driveFileId?`<span class="state-icon" role="img" aria-label="Drive 백업 파일 있음" title="Drive 백업 파일 있음">${icon('cloud')}</span>`:''}<span class="state-icon" role="img" aria-label="${state}" title="${state}">${icon(statusIcon)}</span></div><div class="card-tools"><button class="icon-button" data-card-action="edit" data-entry-id="${id}" aria-label="${title} 수정" title="수정">${icon('edit')}</button>${x.driveFileId?`<button class="icon-button" data-card-action="refresh" data-entry-id="${id}" aria-label="${title} AI 새로고침" title="AI 새로고침">${icon('refresh')}</button>`:''}<button class="icon-button delete-icon" data-card-action="delete" data-entry-id="${id}" aria-label="${title} 삭제" title="삭제">${icon('trash')}</button></div></div><div class="item-title">${title}</div><div class="meta">${esc(x.date)} · ${(x.types||[]).map(esc).join(', ')||'유형 없음'}</div><div class="preview"><b>상황</b> ${esc(x.situation)}\n<b>행동</b> ${esc(x.action)}\n<b>결과</b> ${esc(x.result)}</div><div class="tagrow">${(x.skills||[]).slice(0,5).map(s=>`<span class="tag">${esc(s)}</span>`).join('')}${['높음','핵심 후보'].includes(x.value)?`<span class="tag">${esc(x.value)}</span>`:''}</div></article>`;
...
$('#entries').addEventListener('click',e=>{
  ...
  if(action){const id=action.dataset.entryId;({edit:editItem,refresh:pullOneReview,delete:deleteItem})[action.dataset.cardAction]?.(id);return}
  const card=e.target.closest('[data-card-open]');if(card)openItem(card.dataset.cardOpen);
});
window.openItem=id=>{
